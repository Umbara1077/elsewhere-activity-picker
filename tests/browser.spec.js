...
test('real UI: romantic dinner, reroll, takeout, outdoors, events and narrow radius',async({page})=>{
 ...
 const dialog=page.getByRole('dialog');await expect(dialog.locator('.dialog-title')).toHaveText(/Monalisia|Library IV/,{timeout:12000});
 const first=await dialog.locator('.dialog-title').innerText();await expect(dialog).not.toContainText('Dunkin');
 await dialog.getByRole('button',{name:'Roll again'}).click();await expect(dialog.locator('.dialog-title')).toHaveText(first==='Monalisia'?'Library IV':'Monalisia',{timeout:12000});
 console.log('ROMANTIC PICKS:',first,await dialog.locator('.dialog-title').innerText());
 await dialog.getByRole('button',{name:'Close',exact:true}).click();
 await page.getByRole('button',{name:'Take out',exact:true}).click();await page.getByRole('button',{name:'Surprise me',exact:true}).click();await expect(dialog.locator('.dialog-title')).toHaveText(/Monalisia|Library IV/,{timeout:12000});await dialog.getByRole('button',{name:'Close',exact:true}).click();
 ...
 await page.getByRole('button',{name:'Surprise me',exact:true}).click();await expect(dialog.locator('.dialog-title')).toHaveText("The 40th Annual Kid's Fishing Contest",{timeout:12000});await dialog.getByRole('button',{name:'Close',exact:true}).click();
...
test('08094 still picks with external discovery services unavailable',async({page})=>{
 await page.route(/api\.zippopotam\.us|maps\.mail\.ru|\/api\/discover/,route=>route.abort());
 await page.goto(process.env.TEST_URL||'http://127.0.0.1:5173/');
 ...
 await page.getByRole('button',{name:'Surprise me',exact:true}).click();await expect(page.getByRole('dialog').locator('.dialog-title')).toHaveText(/Monalisia|Library IV/,{timeout:12000});expect(page.context().pages()).toHaveLength(1);
});
