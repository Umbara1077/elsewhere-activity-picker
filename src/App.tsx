...
export default function App() {
 ...
 const update=<K extends keyof Filters>(k:K,v:Filters[K])=>{setFilters(f=>({...f,[k]:v}));setShowAll(false);};
 ...
 async function surprise(){
  ...
  if(!choice){
   setPicking(false);
   setToast(snapshot?'No actual places match those filters. Try a wider mood, budget, or distance.':'I couldn’t reach the local listings. Please retry; I won’t substitute a generic idea.');
   return;
  }
 ...
 return <div className="app-shell">
  ...
  <main>
   ...
   <section className="picker-panel" id="picker" aria-label="Activity picker">
    ...
    <div className="picker-body"><div className="mood-line"><span className="small-heading">Set the mood</span><div className="mood-options">{moods.map(({id,label,icon:Icon})=><button key={id} aria-pressed={filters.mood===id} className={`mood-chip ${filters.mood===id?'selected':''}`} onClick={()=>update('mood',id)}><Icon size={15}/>{label}</button>)}</div></div>
     <form className="search-row" onSubmit={discover}><label className={`search-field location-field ${formError?'invalid':''}`}><MapPin size={20}/><span><span className="field-caption">STARTING FROM</span><input ref={searchRef} value={zip} onChange={e=>setZip(e.target.value.replace(/\D/g,'').slice(0,5))} inputMode="numeric" autoComplete="postal-code" placeholder="Enter your ZIP code" aria-label="US ZIP code" aria-invalid={Boolean(formError)}/></span>{location&&<span className="location-name">{location.city}</span>}</label>
      <label className="search-field"><Navigation size={19}/><span><span className="field-caption">HOW FAR?</span><select value={filters.radius} onChange={e=>update('radius',Number(e.target.value))} aria-label="Search radius">{[5,10,25,50,100].map(n=><option key={n} value={n}>Within {n} miles</option>)}</select></span></label>
   ...
   <section className="discoveries" id="discoveries">
    ...
    {!loading&&displayed.length===0&&<div className="empty-state"><Compass size={36}/><h3>{view==='saved'?'Good plans deserve a bookmark.':'No local matches yet.'}</h3><p>{view==='saved'?'Tap the bookmark on any idea, or add a favorite of your own.':'Try a wider mood, budget, or distance. Places with unknown prices or accessibility are excluded when those filters are set.'}</p><button className="dark-button" onClick={()=>{setFilters(initialFilters);setView('discover');}}>Reset filters <ArrowRight size={17}/></button></div>}
   </section>
